...
char nombre[18], nombre2[18],mano [12][28];;
char mat[17][11];
...
void Inicio();
/*Esta función se encarga de hacer la lectura desde el archivo txt donde yace el título del juego.*/
...
/*La funcion de scenary es aquella que va a desplegar el escenario/tablero al jugar BlackJak. Tiene como parámetros de entrada
los colores que tendrá tanto a su alrededor como al centro.*/
...
void prepa7();
void prepa8();
void prepa9(); //Estas funciones leen los archivos de cada prepa y la dibujan en la pantalla, asigna mensajes a m1,m2,m3,m4ym5
...
void menudf();//mapa y menú de la CDMX
void ganarfinal(); //pantalla cuando se gana el modo historia

int main(){
  system("mode con cols=170 lines=60"); //agrandamos la consola
  marco(91,17,34,14,2,12,2); //mándamos un mensaje al inicio para que el usuario configure el tamaño de su pantala
  system("CLS");
  Sleep(750);
  Inicio(); //Mándamos a pantalla el título del juego 
  PrincipalFrame(); //Menú de inicio
  return 0;
}
void gotoxy1(int x,int y){  
  HANDLE handle;  
...
void lecturaInstrucciones(){
     ...
     int i, c, v, k;
     int coordenadas[8][5] = {{2, 3, 1, 2, 2}, {2, 14, 4, 3, 1},{2, 25, 6, 1, 2},{2, 36, 8, 4, 1},{130, 3, 3, 1, 2},{130, 14, 2, 1, 2},{130, 25, 11, 1, 2},{130, 36, 13, 2, 2}};
     i=0;
     FILE * archivo;
     ...
     fclose(archivo); 
     textcolor(12);
     printf("\n\n                                         Para ver los valores de las cartas presione tecla 'V'");
     textcolor(9);
     printf("\n\n                                        Presione cualquier tecla para volver al men%c de inicio", 233);
     printf("\n\n                                                                EXIT"); 
     printf("  <-| ");
     for(v = 0; v<8 ; v++){//Con este for asignamos las coordenadas, valores, símbolos y colores a las cartas que desplegamos en la interfaz de las instrucciones.
               cartas[v].CoordX = coordenadas[v][0];
               cartas[v].CoordY = coordenadas[v][1]; 
               cartas[v].randNum = coordenadas[v][2];
               cartas[v].randSim = coordenadas[v][3];
               cartas[v].color = coordenadas[v][4];
     }
     for(k = 0; k<8; k++)
         CrearCarta(k,0,0);  
...
void valorCartas(){
     int  k, v, r;
     char Arch[1000], c, opcion;
     int coordenadas[6][5] = {{15, 3, 1, 2, 2}, {15, 21, 10, 3, 1}, {60, 3, 11, 1, 2}, {60, 21, 12, 4, 1}, {107, 3, 13, 1, 2}, {107, 21, 6, 2, 2}};
     FILE * archivo;
     archivo=fopen("c:\\cartas\\valores.txt", "r");
     while((c=fgetc(archivo)) != EOF)
	 {
          if(c=='\n') 
              printf("\n"); 
     ...
     fclose(archivo); 
     for(v = 0; v<6 ; v++){//Con este for asignamos las coordenadas, valores, símbolos y colores a las cartas que desplegamos en la parte de las instrucciones enseñando los valores de las cartas.
               cartas[v].CoordX = coordenadas[v][0];
               cartas[v].CoordY = coordenadas[v][1]; 
               cartas[v].randNum = coordenadas[v][2];
               cartas[v].randSim = coordenadas[v][3];
...
void PrincipalFrame(){
     ...
     char  letra, TEC;
     int l=0, i=0, o=0, k=0;
     int coordenadas[8][5] = {{4, 3, 1, 2, 2}, {6, 15, 10, 3, 1}, {8, 27, 11, 1, 2}, {125, 3, 13, 1, 2}, {123, 15, 6, 2, 2}, {121, 27, 3, 2, 1}};
     int x=82,y=22;
     for(i=0; i<25; i++)
	 {
          for(o=0; o<20; o++)
	 ...
	 gotoxy1(44,45);
	 textcolor(12);
	 printf(" Para seleccionar otra opcion ya seleccionada, presionar 'Z' ");
     gotoxy1(x,y); 
	 textbackground(11); 
	 textcolor(15); 
	 printf("<-");
	 for(v = 0; v<6 ; v++){ //Con este for asignamos las coordenadas, valores, símbolos y colores a las cartas que desplegamos al inicio. 
               cartas[v].CoordX = coordenadas[v][0];
               cartas[v].CoordY = coordenadas[v][1]; 
               cartas[v].randNum = coordenadas[v][2];
               cartas[v].randSim = coordenadas[v][3];
...
void Inicio(){
     ...
     while(!feof(archivo)) //Asignación de los valores del archivo .txt a la matriz para después desplegarlo al inicio. 
	 {
            tecla=fgetc(archivo);
     ...
     for(i=0; i<16; i++){
        Sleep(8);       
        for(o=0; o<87; o++){ //Estos fors imrpimen el mensaje de "Casino Coyote" con sus respectivos colores.
           gotoxy1(32+o,3+i);
...
void Mensaje(int NumMensaje){ //mensajes cuando se gana o se pierde
...
void scenary(int color1, int color2, int color3){
    ...
    for(i=0; i<17; i++) //Estos fors van a imprimir la matriz de Tab que contiene el diseño del tablero. Va a imprimir distintos colores en función a los parámetros de entrada y a los números en la matriz.  
	{
        for(o=0; o<126; o++)
		{
           gotoxy1(28+o,12+i);
...
void DefBaraja(){ 
     ...
     division=false;
     /*En los ifs a continuación se van a retornar unas variables a verdadero dependiendo los casos para poder asegurar en la partida o para poder dividir*/
...
int ValidacionAs(int valCg, int NoC, int p){
       /*Dependiendo los casos en los que puede salir la carta "A", estos ifs van a evaluarlos y reasignarle el valor de uno u once, según lo más conveniente para el usuario.*/
       if(NoC == 0 || NoC == 16)
	   {
		   Player[p].BoR4=true; 
		   return 11;
...
void animacion(int numcartas, int J){
     int i= 0, n= 0;
     if(J==0){ //En este if se hará la animación de las cartas del usuario, las cuales son más grandes que las de la computadora.  
     ...
     else if(J==2){//En este if se hará la animación de las cartas de la computadora. 
        textbackground(15);
     ...
     else if(J==3){//En este if se borrará la carta. Se emplea cuando se divide para borrar la primera carta y transferirla a la segunda mano.   
        gotoxy1(cartas[numcartas].CoordX, cartas[numcartas].CoordY);
        textbackground(0);
        for(n=cartas[numcartas].CoordY; n<= cartas[numcartas].CoordY+13; n++){
                   for(i= cartas[numcartas].CoordX; i<= cartas[numcartas].CoordX + 16; i++){ 
     ...
     if(Comenzarajugar == true)
           Sleep(200);
     return;      
}
void CleanCrdS(){ 
        int i=0, o=0;
		for(o=1; o<=153; o++)//En estos fors se van a limpiar las cartas anteriores, una vez terminada la partida. 
		{
...
void dividir(){
    ...
    textbackground(0);
    boolJuegoDiv=false;
    if(flecha==13 && y==10)//el usuario dividió
	{ 
                  animacion(0,3); 
...
void Aseg(){
	...
	{    
     ...
     if(flecha==13 && y==5)//el usuario aseguró
     {	    
           AsegurarAp=true;  
           limpiarmsj(); 
           n3=apuesta1(n1,1);
     }                                  
     else if(flecha==13 && y==7)//el usuario no aseguró
    	AsegurarAp=false;
     limpiarmsj();
     textbackground(0);
     for(i=3; i<9; i++)
...
void CrearCarta(int NumCarta, int Jugador, int estructuraElementos){ 
     ...
     arch= fopen(nomar, "r");
     int col = 0, ren = 0;
     while(!feof(arch)) //En este while se está asignando los valores el archivo .txt a la matriz para después poder desplegarla.
        {
             tecla = fgetc(arch);
             if(Jugador == 0) 
     ...
     if(cartas[NumCarta].randNum != 1 && cartas[NumCarta].randNum != 11 && cartas[NumCarta].randNum != 12 && cartas[NumCarta].randNum != 13)
		...
		{ 
			(cartas[NumCarta].color == 1)? textcolor(0): textcolor(12); 
			printf("A "); 
			cartas[NumCarta].randNum = ValidacionAs(Player[Jugador].valChng, NumCarta, estructuraElementos);//Aquí se manda a llamar la función de ValidacionAs misma que retornará a la carta de "A" el calor de uno u once. 
...
void AlgJueComp(){
     ...
     while(Player[1].conteoCartas<48 && Player[1].sumaNum<17 && ((Player[0].sumaNum <=21 && boolJuegoDiv == false) || ((Player[0].sumaNum <=21 || Player[2].sumaNum <=21) && boolJuegoDiv == true)))
	 {
			...
			/*En este if se va a llevar a cabo la resta de 10 para retornar el valor de una carta "A" de once a uno.*/
            ...
            Player[1].conteoCartas++; //El conteo de cartas va a ir aumentando a la par que vayas solicitando cartas para que se pueda generar la carta correspondiente con la función de CrearCarta.
     ...
     while((jugador == 0 && boolJuegoDiv == false) ? (tecla != 13 && Player[jugador].conteoCartas<16 && Player[jugador].sumaNum<21) : ( (jugador == 0 && boolJuegoDiv == true) ? (Player[jugador].conteoCartas <3) : (Player[jugador].conteoCartas < 18)) )
	 {
	     if(kbhit())
		 {
	        ...
	        if(tecla=='a' || tecla=='A')
			{
				...
				{ 
					dividir();
                    //Este if va a desplegar las dos primeras cartas si es que se llega a dividir. Para que el usuario únicamente pueda verlas antes de solicitar las demás.  
           ...
           //Si se llega a presionar la tecla 'D', se va a doblar. 
...
void DefGan(int S0, int S1, int S2){//condiciones para ganar, empatar o perder
       if(S0>0)
       {
           if(S0<=21)
           {
...
void menuPregunta(){
     ...
     if(TEC==13 && y ==20)//el usuario quiere seguir jugando
     { 
          Comenzarajugar=true;
          gotoxy1(78, 25); 
          textcolor(4);
     ...
     else if(TEC==13 && y ==23)//el usuario ya no desea jugar
     {
          Comenzarajugar=false;
          textbackground(0);
          system("CLS");
          ...
          {
             FILE *archivo;
             archivo=fopen("c:\\cartas\\Jugadores.txt", "a");//se guardan los puntajes
...
void borrarmano(int condic)//quita la mano de la pantalla
{ 
 int i=0, j=0;
...
void bormat()//función para borrar las fichas
{
int i=0, j=0;
for(i = 0; i < 12; i++)
      {
...
void manita(int crx, int cry){//lectura de la mano de apuesta por medio de una matriz que se encuentra en un archivo de texto
int p1, p2, i=0, j=0;
FILE * dibman;
dibman=fopen("c:\\cartas\\manita.txt","r");
    char tecla1;
    while (!feof(dibman))
    {
        while(i<=12)
...
void lecturas (int mat[][24], int cdx, int cdy, char loc[] )//lee los archivos de montones de fichas y los guarda en ciertas partes de la matriz que se define en funcion fichas
{ 
     char tecla;
     FILE *fichi; 
     int y=0, x;
...
void impfich(int mat[][24], int con)//colores y carácteres de las fichas
{
	    int i=0, j=0;
	    if(con==2)
              manita(120,14);
...
{
	    char location[30];
		if(confich==5)//se abren los archivos de fichas
		{
		     if(numrand==1)
             {
...
{ 
    ...
    nr=rand()%3+1;
    xf=num;
	if(nfich>=8&&nfich<16)//se mandan los montos según la cantidad apostada
    {
       xf=num-8;	
	   yf=yf+2;
    }
...
{
  int x=17, y=6,apuesta=0, cfich=0, apuestainv=0, i=0,a=0,c=0; 
    z=apmin;
    char flecha;
    textbackground(0);
  do
   ...
   do
  {
    ...
    textcolor(15); 
    printf("<-"); 
      while(flecha!=13 || y!=18)// menú de apuesta
      {
          textbackground(0);
          textcolor(15); 
          gotoxy1(8,2);
          ...
          if(kbhit()){
                   ...
                   textcolor(15); 
                   printf("<-");   
                   if(flecha==13 && y==6)//montos de apuestas
                         {
        ...
        if(apmin>apuesta)
      {
            ...
            gotoxy1(50,10);
            textbackground(0);
            printf("Te faltan pumadolares, espera a que llegue prepa s%c",161);//mensajes de error
            Sleep(3000);
                        for(i=17;i<=23; i++)
                                        {
                                            gotoxy1(i,2); 
  ...
  while(apmax<apuesta||apmin>apuesta||AsegurarAp==true && apuesta>n1/2||apuesta>dinjugador && apuesta<=apmax);//se ejecuta el do siempre que haya un error en la apuesta
  gotoxy1(2,20);
...
int doblar(int apuesta){
    int apuesta2=apuesta;//duplicación de la apuesta
    if(doblarr==true)
{
    dinjugador=dinjugador-apuesta;
    apuesta2=apuesta*2;
...
void dinero(){
     if(bool0win==true||blackjack0==true && boolJuegoDiv==true) //Se llevan acabo todas las ganancias
     {
          n1*=2;
     ...
     {
          n1*= 1.5;//se multiplica una variable int por un float para que se redonde al final y nos de un numero entero
          dinjugador=dinjugador+n1;
          blackjack0=false;              
     }
     if (bool2win==true)
...
{ 
  ...
  while(Comenzarajugar==true)
	{
     textbackground(2);//diseño de las impresiones del tablero
     textcolor(15);
     gotoxy1(80,20);
     printf("Blackjack se paga 3 a 2");
     gotoxy1(81,21);
     printf("cualquier otra 1 a 2");
     textcolor(0);
     gotoxy1(45,26);
     textbackground(14);
     ...
     n1=apuesta1(v,b);
     {
     Score0=0, Score1=0, Score2=0;//se inicializan la suma de cada jugador
     ...
     if(dinjugador>0)
                 menudf();
     if(dinjugador==0)  //animación game over    
           {      
                 system("CLS");
                 Mensaje(3);
                 system("CLS");
...
void impmen( char cad[], int tam)
{
     ...
     {    
          prub=cad[x];
          if(prub!='#'&&prub!='*'&&prub!='%'&&prub!='$'&&prub!='-'&&prub!=':'&&prub!='&'&&prub!='='&& prub!='+'&& prub!='{')
          {
          gotoxy1(x,41);
          printf("%c",prub);
          }
          ...
          {
               if(prub=='#')//acentos para los mensajes
               {
                  gotoxy1(x,41);
                  printf("%c",160);
               }
...
void lectur(char nprepa[2])
{
    ...
    while (!feof(dib))
    {
        while(y<=40)
        {
            ...
            {
                tecla=fgetc(dib);
                if(tecla=='0'||tecla=='1'||tecla=='2'||tecla=='3'||tecla=='4'||tecla=='5'||tecla=='6'||tecla=='7'||tecla=='8'||tecla=='9'||tecla=='a'||tecla=='b'||tecla=='c'||tecla=='d'||tecla=='e'||tecla=='f' ||tecla=='g'||tecla=='h'||tecla=='i')
                tabla[y][x]=tecla;
            }
            y++;
        }
	...
	while(f!=13)
    	{
         ...
         if (color2==14)
             color2--;
         Sleep(50);
         marco(24,39,0,0,1,color1,color2);
         if(color1==15||color2==0)
         {
             color1=0;
             color2=15;
        }
...
{ 
    int c;
	for (c=0;c<=100;c++)//se limpian los mensajes
	    m1[c]=' ';
    for (c=0;c<=100;c++)
	    m2[c]=' ';
	for (c=0;c<=100;c++)
...
void prepa1 ()
{
	borrarmensajes (); //se limpia la consola de mensajes
    strcpy(np, "1");  
    strcpy(m1, "Por aqu% anda el chavo con el pase de acceso para jugar en la competencia de Blackjack, b-scalo"); //mensajes de los personajes
...
{
    system("CLS");
    lectur(np);//lee el mapa de la prepa
    marco(24,39,0,0,1,1,6); //marco de color
	monito(67,137,18,40, 79, 127, 89, 98, 115, 23, 35, 28, 30, 25,0);//coordenadas de los personajes y de la delimitación de movimiento
	Comenzarajugar=true;
    scenary(2,15,12); //se manda llamar al juego
    Juego(500,1);
}
	while(Comenzarajugar==false && dinjugador>=10); //se ejecuta lo del do sólo cuando el usuario tiene el dinero suficiente para la prepa
	if(dinjugador<10)
	  menudf(); //si se sale del juego, se regresa al menú
	return;
}
void prepa2 ()
{
	...
	do
{
	...
	monito(28,102,12,28, 89, 78, 46, 67, 97, 15, 24, 21, 18, 19,0);
	Comenzarajugar=true;
    scenary(2,7,12);
    Juego(1000,200);
}
...
void prepa3 ()
{
	...
	strcpy(m3, "::::COMBATE, ACCI=N Y GUERRA, COMBATE ACCI=N Y GUERRA ARRIBA LA PREPA JUSTO SIERRAAA!!!!!");
...
void prepa5 ()
{
    ...
    strcpy(m4, "&Sab%as que la prepa 5 es la prepa m*s grande de Latinoam*rica, alberga mas de 100000 estudiantes?");
    strcpy(m5, "S% deseas jugar presiona ENTER, si no, presiona cualquier tecla ");
	do
{
    system("CLS");
    lectur(np);
	...
	do
{
	...
	monito(49,139 , 24,41 ,120 , 117, 55, 87,78, 37, 32, 25, 29, 35 ,0);
	Comenzarajugar=true;
    scenary(2,7,12);
    Juego(7000,2000);
}
...
void prepa9 ()
{
    ...
    bprepa9=true;
    strcpy(np, "9");
    strcpy(m1, "Para retar al totem, necesitas tener 37000 pumad$lares y presionar ENTER, deber#s apostar todo,si me engañas, te enga.ar*");
    ...
    while(flecha!=13)
      {
          if(kbhit()){
                   ...
                   textcolor(15); 
                   printf("<-");
                   if(flecha=='e'||flecha=='E') //se guardan los datos de los jugadores al salir
                   {
                        modohistoria=false;
                        FILE *archivo;
                        archivo=fopen("c:\\cartas\\Jugadores.txt", "a");
                        fprintf(archivo, "\n%s acumul%c: %i pumad%clares", nombre2, 162, dinjugador, 162);
                        fclose(archivo);
                        system("CLS");
                        Inicio();
                        PrincipalFrame();
...
void marco(int xm, int ym, int xi,int yi, int con, int color, int fon)
{
    ...
    if(con==2){
     gotoxy1(35,15);
     textcolor(15);
     printf("Maximiza la pantalla y luego presiona cualquier tecla :)");
     getch();
    }
     textbackground(0);
     return;
